...
int main(int _argc, char **_argv){
    ...
    strncpy(__sh_script_name, "b5_control_flow.sh", 65530);

    __sh_lineno=3;
    {
    __sh_puts("=== B5: control flow boundary ===");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=6;
    strncpy(val,"maybe",sizeof(val)-1);
    __sh_lineno=7;
    if((strcmp(__sh_fmt("%s",val), "yes") == 0)||(strcmp(__sh_fmt("%s",val), "y") == 0)){
    __sh_lineno=8;
    {
    __sh_puts("affirmative");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    } else if((strcmp(__sh_fmt("%s",val), "no") == 0)||(strcmp(__sh_fmt("%s",val), "n") == 0)){
    __sh_lineno=9;
    {
    __sh_puts("negative");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    } else if((strcmp(__sh_fmt("%s",val), "maybe") == 0)){
    __sh_lineno=10;
    {
    __sh_puts("uncertain");
    }
    ...
    __sh_lineno=27;
    {
    const char *__fl27[] = {"1", "2", "3", "4", "5", NULL};
    for (int __fi27 = 0; __fl27[__fi27]; __fi27++) {
        strncpy(i, __fl27[__fi27], sizeof(i) - 1);
    __sh_lineno=28;
    if(((atoi(i) == 3))){
    __sh_lineno=29;
    continue;
    ...
    __sh_lineno=34;
    {
    char __tw_0[1024]; snprintf(__tw_0, sizeof(__tw_0), "loop=%s", i);
    __sh_putf("%s", __tw_0);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    __sh_lineno=38;
    {
    const char *__fl38[] = {"1", "2", "3", NULL};
    for (int __fi38 = 0; __fl38[__fi38]; __fi38++) {
        strncpy(i, __fl38[__fi38], sizeof(i) - 1);
    __sh_lineno=39;
    {
    const char *__fl39[] = {"a", "b", "c", NULL};
    for (int __fi39 = 0; __fl39[__fi39]; __fi39++) {
        strncpy(j, __fl39[__fi39], sizeof(j) - 1);
    __sh_lineno=40;
    if(((strcmp(j, "b") == 0))){
    __sh_lineno=41;
    break;
    ...
    __sh_lineno=43;
    {
    char __tw_1[1024]; snprintf(__tw_1, sizeof(__tw_1), "outer=%s inner=%s", i, j);
    __sh_putf("%s", __tw_1);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    __sh_lineno=56;
    {
    char __tw_2[1024]; snprintf(__tw_2, sizeof(__tw_2), "while_n=%d", n);
    __sh_putf("%s", __tw_2);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=57;
    {
    { long long __arith_rc=(long long)(n++); __exit_status=__arith_rc?0:1; }
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
