...
static const char *__sh_proc_subst(const char *cmd, char dir){
  ...
  if(pid==0){
    ...
    /* Use execvp instead of execl sh -c, to avoid external shell dependency */
    ...
    while(tk&&ai<127){
      ...
      int sl=strlen(tk);
      if(sl>=2 && ((tk[0]=='"'&&tk[sl-1]=='"')||(tk[0]=='\''&&tk[sl-1]=='\''))){
        tk++; tk[sl-2]=0;
      }
      av[ai++]=tk; tk=strtok(NULL," \t");
    } av[ai]=NULL;
...
static void __sh_puts(const char *s){ if(__sh_div_error){__sh_div_error=0;return;} /* Use write() directly to avoid glibc stdio buffer leaking plaintext */ size_t _sl=strlen(s); write(1,s,_sl); write(1,"\n",1); }
...
/* ---- user variables ---- */
static char __sh_arg0[65536]="";
static char __sh_arg1[65536]="";
static char __sh_arg2[65536]="";
static char __sh_arg3[65536]="";
static char __sh_arg4[65536]="";
static char __sh_arg5[65536]="";
static char __sh_arg6[65536]="";
static char __sh_arg7[65536]="";
static char __sh_arg8[65536]="";
static char __sh_arg9[65536]="";
static const char *__arr_empty[256]={NULL};
static char e[65536]="";
static const char *__arr_arr[256]={NULL};
static const char *__arr_sparse[256]={NULL};
static const char *__arr_arr2[256]={NULL};
static const char *__arr_arr3[256]={NULL};
static const char *__arr_m[256]={NULL};
static const char *__arr_arr4[256]={NULL};
static char _sh_'arr4[2]'[65536]="";
...
int main(int _argc, char **_argv){
    ...
    __sh_start_time=time(NULL);
    __sh_argc = _argc - 1;
    __sh_args = _argv + 1;
    if (_argc > 0) strncpy(__sh_arg0, _argv[0], 4095);
    if (_argc > 1) strncpy(__sh_arg1, _argv[1], 4095);
    if (_argc > 2) strncpy(__sh_arg2, _argv[2], 4095);
    if (_argc > 3) strncpy(__sh_arg3, _argv[3], 4095);
    if (_argc > 4) strncpy(__sh_arg4, _argv[4], 4095);
    if (_argc > 5) strncpy(__sh_arg5, _argv[5], 4095);
    if (_argc > 6) strncpy(__sh_arg6, _argv[6], 4095);
    ...
    strncpy(__sh_script_name, "b8_array_extreme.sh", 65530);

    __sh_lineno=5;
    /* array empty */
    {
    int __an5=0;
    __arr_empty[__an5]=NULL;
    }
    __sh_lineno=6;
    {
    for(int __ai=0;__arr_empty[__ai];__ai++){
    strncpy(e,__arr_empty[__ai],sizeof(e)-1);
    __sh_lineno=7;
    {
    char __tw_0[1024]; snprintf(__tw_0, sizeof(__tw_0), "should_not_print: %s", e);
    __sh_putf("%s", __tw_0);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    }
    __sh_lineno=9;
    {
    __sh_puts("empty_done");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=12;
    /* array arr */
    {
    int __an12=0;
    __arr_arr[__an12++]=strdup("a");
    __arr_arr[__an12++]=strdup("b");
    __arr_arr[__an12++]=strdup("c");
    __arr_arr[__an12++]=strdup("d");
    __arr_arr[__an12++]=strdup("e");
    __arr_arr[__an12++]=strdup("f");
    __arr_arr[__an12++]=strdup("g");
    __arr_arr[__an12++]=strdup("h");
    __arr_arr[__an12++]=strdup("i");
    __arr_arr[__an12++]=strdup("j");
    __arr_arr[__an12]=NULL;
    }
    __sh_lineno=13;
    {
    char __tw_1[1024]; snprintf(__tw_1, sizeof(__tw_1), "slice0=%s", __sh_arr_slice(__arr_arr,0,0));
    __sh_putf("%s", __tw_1);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=14;
    {
    char __tw_2[1024]; snprintf(__tw_2, sizeof(__tw_2), "slice_neg=%s", __sh_arr_slice(__arr_arr, -3,-1));
    __sh_putf("%s", __tw_2);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=15;
    {
    char __tw_3[1024]; snprintf(__tw_3, sizeof(__tw_3), "slice_oob=%s", __sh_arr_slice(__arr_arr,100,3));
    __sh_putf("%s", __tw_3);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=16;
    {
    char __tw_4[1024]; snprintf(__tw_4, sizeof(__tw_4), "slice_zero_len=%s", __sh_arr_slice(__arr_arr,2,0));
    __sh_putf("%s", __tw_4);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=19;
    {
    char __tw_5[1024]; snprintf(__tw_5, sizeof(__tw_5), "neg1=%s", ((__sh_arr_count(__arr_arr)+(-1)>=0)?(__arr_arr[__sh_arr_count(__arr_arr)+(-1)]?__arr_arr[__sh_arr_count(__arr_arr)+(-1)]:""):(fprintf(stderr,"%s: line %d: arr: bad array subscript\n",__sh_script_name[0]?__sh_script_name:"shell2c",__sh_lineno),"")));
    __sh_putf("%s", __tw_5);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=20;
    {
    char __tw_6[1024]; snprintf(__tw_6, sizeof(__tw_6), "neg_full=%s", __sh_arr_slice(__arr_arr, -1,-1));
    __sh_putf("%s", __tw_6);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    { int __ai=0; while(__arr_sparse[__ai]) __ai++;
    int __idx=atoi("0");
    if(__idx>=256) __idx=255;
    __arr_sparse[__idx]=strdup("a");
    }
    { int __ai=0; while(__arr_sparse[__ai]) __ai++;
    int __idx=atoi("3");
    if(__idx>=256) __idx=255;
    __arr_sparse[__idx]=strdup("b");
    }
    { int __ai=0; while(__arr_sparse[__ai]) __ai++;
    int __idx=atoi("7");
    if(__idx>=256) __idx=255;
    __arr_sparse[__idx]=strdup("c");
    }
    __sh_lineno=26;
    {
    char __tw_7[1024]; snprintf(__tw_7, sizeof(__tw_7), "sparse_count=%d", __sh_arr_count(__arr_sparse));
    __sh_putf("%s", __tw_7);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=27;
    {
    char __tw_8[1024]; snprintf(__tw_8, sizeof(__tw_8), "sparse_idx=%s", __sh_arr_indices(__arr_sparse));
    __sh_putf("%s", __tw_8);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=30;
    ...
    {
    int __an30=0;
    __arr_arr2[__an30++]=strdup("x");
    __arr_arr2[__an30++]=strdup("y");
    __arr_arr2[__an30++]=strdup("z");
    __arr_arr2[__an30]=NULL;
    }
    __sh_lineno=31;
    /* array arr2 */
    {
    int __an31=0;
    __arr_arr2[__an31++]=strdup("a");
    __arr_arr2[__an31++]=strdup("b");
    __arr_arr2[__an31++]=strdup("c");
    __arr_arr2[__an31++]=strdup("d");
    __arr_arr2[__an31]=NULL;
    }
    __sh_lineno=32;
    {
    char __tw_9[1024]; snprintf(__tw_9, sizeof(__tw_9), "reassign_count=%d", __sh_arr_count(__arr_arr2));
    __sh_putf("%s", __tw_9);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=33;
    {
    char __tw_10[1024]; snprintf(__tw_10, sizeof(__tw_10), "reassign_all=%s", __sh_arr_join(__arr_arr2));
    __sh_putf("%s", __tw_10);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=36;
    ...
    {
    int __an36=0;
    __arr_arr3[__an36++]=strdup("hello");
    __arr_arr3[__an36++]=strdup("world");
    __arr_arr3[__an36++]=strdup("test");
    __arr_arr3[__an36]=NULL;
    }
    __sh_lineno=37;
    {
    char __tw_11[1024]; snprintf(__tw_11, sizeof(__tw_11), "arr_len=%d", __sh_arr_count(__arr_arr3));
    __sh_putf("%s", __tw_11);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=38;
    {
    char __tw_12[1024]; snprintf(__tw_12, sizeof(__tw_12), "elem0_len=%d", 1);
    __sh_putf("%s", __tw_12);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=39;
    {
    char __tw_13[1024]; snprintf(__tw_13, sizeof(__tw_13), "elem1_len=%d", 1);
    __sh_putf("%s", __tw_13);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=42;
    __sh_lineno=43;
    {
    char __tw_14[1024]; snprintf(__tw_14, sizeof(__tw_14), "empty_assoc=%d", __sh_arr_count(__arr_m));
    __sh_putf("%s", __tw_14);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_assoc_set(__arr_m,"key1",strdup("val1"));
    __sh_assoc_set(__arr_m,"key2",strdup("val2"));
    __sh_lineno=46;
    {
    char __tw_15[1024]; snprintf(__tw_15, sizeof(__tw_15), "assoc_keys=%s", __sh_arr_indices(__arr_m));
    __sh_putf("%s", __tw_15);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=47;
    {
    char __tw_16[1024]; snprintf(__tw_16, sizeof(__tw_16), "assoc_vals=%s", __sh_arr_join(__arr_m));
    __sh_putf("%s", __tw_16);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=50;
    ...
    {
    int __an50=0;
    __arr_arr4[__an50++]=strdup("a");
    __arr_arr4[__an50++]=strdup("b");
    __arr_arr4[__an50++]=strdup("c");
    __arr_arr4[__an50++]=strdup("d");
    __arr_arr4[__an50++]=strdup("e");
    __arr_arr4[__an50]=NULL;
    }
    __sh_lineno=51;
    {
    { int __ui=atoi("2"); if(__ui>=0 && __ui<256) { __arr_arr4[__ui]=NULL; } }
    { int __pi=0,__pj=0; for(;__pi<256;__pi++){ if(__arr_arr4[__pi]){ if(__pi!=__pj) __arr_arr4[__pj]=__arr_arr4[__pi]; __pj++; } } __arr_arr4[__pj]=NULL; }
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=52;
    {
    char __tw_17[1024]; snprintf(__tw_17, sizeof(__tw_17), "after_unset=%s", __sh_arr_join(__arr_arr4));
    __sh_putf("%s", __tw_17);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=53;
    {
    char __tw_18[1024]; snprintf(__tw_18, sizeof(__tw_18), "after_unset_count=%d", __sh_arr_count(__arr_arr4));
    __sh_putf("%s", __tw_18);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
