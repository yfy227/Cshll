...
static char inner[65536]="";
static char opt[65536]="";
static char x[65536]="";
static char num[65536]="";
static char color[65536]="";

/* ---- function declarations ---- */
static void find_item(int,char**);

/* ---- function definitions ---- */
static void find_item(int __sh_argc, char **__sh_args){
    char __sh_arg1[65536]=""; if(__sh_argc>=1)strncpy(__sh_arg1,__sh_args[1-1],65535);
    char __sh_arg2[65536]=""; if(__sh_argc>=2)strncpy(__sh_arg2,__sh_args[2-1],65535);
    char __sh_arg3[65536]=""; if(__sh_argc>=3)strncpy(__sh_arg3,__sh_args[3-1],65535);
    __sh_lineno=57;
    {
    char __tw_0[1024]; snprintf(__tw_0, sizeof(__tw_0), "%s", __sh_join_args(__sh_argc,__sh_args));
    char __fl57_buf[8192]; snprintf(__fl57_buf,sizeof(__fl57_buf),"%s",__tw_0);
    const char *__fl57[256]; int __nfl57=0;
    { char *__s=__fl57_buf; char *__tk=strtok(__s," ");
      while(__tk&&__nfl57<255){__fl57[__nfl57++]=__tk;__tk=strtok(NULL," ");} }
    __fl57[__nfl57]=NULL;
    for (int __fi57 = 0; __fl57[__fi57]; __fi57++) {
        strncpy(i, __fl57[__fi57], sizeof(i) - 1);
    __sh_lineno=58;
    if(((strcmp(i, "target") == 0))){
    __sh_lineno=59;
    {
    __sh_puts("found");
    }
...
int main(int _argc, char **_argv){
    ...
    strncpy(__sh_script_name, "b9_control_extreme.sh", 65530);

    __sh_lineno=5;
    {
    const char *__fl5[] = {"1", "2", "3", NULL};
    for (int __fi5 = 0; __fl5[__fi5]; __fi5++) {
        strncpy(i, __fl5[__fi5], sizeof(i) - 1);
    __sh_lineno=6;
    {
    const char *__fl6[] = {"a", "b", "c", NULL};
    for (int __fi6 = 0; __fl6[__fi6]; __fi6++) {
        strncpy(j, __fl6[__fi6], sizeof(j) - 1);
    __sh_lineno=7;
    if(((strcmp(j, "b") == 0))){
    __sh_lineno=8;
    break;
    ...
    __sh_lineno=10;
    {
    char __tw_1[1024]; snprintf(__tw_1, sizeof(__tw_1), "i=%s j=%s", i, j);
    __sh_putf("%s", __tw_1);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    __sh_lineno=13;
    {
    __sh_puts("after_break2");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=16;
    {
    const char *__fl16[] = {"1", "2", "3", NULL};
    for (int __fi16 = 0; __fl16[__fi16]; __fi16++) {
        strncpy(i, __fl16[__fi16], sizeof(i) - 1);
    __sh_lineno=17;
    {
    const char *__fl17[] = {"1", "2", "3", NULL};
    for (int __fi17 = 0; __fl17[__fi17]; __fi17++) {
        strncpy(j, __fl17[__fi17], sizeof(j) - 1);
    __sh_lineno=18;
    if(((atoi(j) == 2))){
    __sh_lineno=19;
    continue;
    ...
    __sh_lineno=21;
    {
    char __tw_2[1024]; snprintf(__tw_2, sizeof(__tw_2), "i=%s j=%s", i, j);
    __sh_putf("%s", __tw_2);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    __sh_lineno=24;
    {
    __sh_puts("after_continue2");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=27;
    strncpy(val,"outer1",sizeof(val)-1);
    __sh_lineno=28;
    if((strcmp(__sh_fmt("%s",val), "outer1") == 0)){
    __sh_lineno=30;
    strncpy(inner,"inner1",sizeof(inner)-1);
    __sh_lineno=31;
    if((strcmp(__sh_fmt("%s",inner), "inner1") == 0)){
    __sh_lineno=32;
    {
    __sh_puts("nested_case_ok");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    } else {
    __sh_lineno=33;
    {
    __sh_puts("nested_case_fail");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    } else {
    __sh_lineno=36;
    {
    __sh_puts("outer_default");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    __sh_lineno=40;
    {
    const char *__fl40[] = {"1", "2", "3", NULL};
    for (int __fi40 = 0; __fl40[__fi40]; __fi40++) {
        strncpy(i, __fl40[__fi40], sizeof(i) - 1);
    __sh_lineno=40;
    {
    { char __cmd40[8192]; int __cl=0;
    __cl+=snprintf(__cmd40+__cl,sizeof(__cmd40)-__cl,"%s",":");
    __exit_status=system(__cmd40);
    if(WIFEXITED(__exit_status))__exit_status=WEXITSTATUS(__exit_status);
    }
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    __sh_lineno=41;
    {
    char __tw_3[1024]; snprintf(__tw_3, sizeof(__tw_3), "empty_loop=%s", i);
    __sh_putf("%s", __tw_3);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=44;
    strncpy(opt,"quit",sizeof(opt)-1);
    __sh_lineno=45;
    if((strcmp(__sh_fmt("%s",opt), "quit") == 0)){
    __sh_lineno=46;
    {
    __sh_puts("quitting");
    }
    ...
    __sh_lineno=47;
    {
    __sh_puts("unknown option");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    __sh_lineno=51;
    snprintf(x,sizeof(x),"%s","5");
    __sh_lineno=52;
    __sh_lineno=52;
    __sh_lineno=52;
    {
    __exit_status=(((atoi(x) > 3)))?0:1;
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    if(__exit_status==0){
    __sh_lineno=52;
    strncpy(x,"big",sizeof(x)-1);
    }
    if(__exit_status!=0){
    __sh_lineno=52;
    strncpy(x,"small",sizeof(x)-1);
    }
    __sh_lineno=53;
    {
    char __tw_4[1024]; snprintf(__tw_4, sizeof(__tw_4), "cond_assign=%s", x);
    __sh_putf("%s", __tw_4);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=56;
    ...
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=70;
    { int __ppfd[2]; if(pipe(__ppfd)<0){perror("pipe");exit(1);}
    pid_t __ppid=fork();
    if(__ppid==0){ close(__ppfd[0]); dup2(__ppfd[1],1); close(__ppfd[1]);
    __sh_lineno=70;
    {
    __sh_echo_escape("1 red\\n2 blue\\n3 green");
    putchar('\n');
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    fflush(stdout); _exit(0); }
    close(__ppfd[1]); int __psi=dup(0); dup2(__ppfd[0],0); close(__ppfd[0]); clearerr(stdin);
    { char __rb[65536];
    while(fgets(__rb,sizeof(__rb),stdin)){
    int __rl=(int)strlen(__rb); if(__rl>0&&__rb[__rl-1]=='\n')__rb[--__rl]=0;
    char *__sp=__rb; int __vi=0; char *__tok=strtok(__sp," \t");
    while(__tok&&__vi<2){ switch(__vi){
    case 0: strncpy(num,__tok,sizeof(num)-1); break;
    case 1: strncpy(color,__tok,sizeof(color)-1); break;
    } __vi++; __tok=strtok(NULL," \t"); }
    __sh_lineno=71;
    if((strcmp(color, "red") == 0)){
    __sh_lineno=72;
    {
    char __tw_5[1024]; snprintf(__tw_5, sizeof(__tw_5), "stop=%s", num);
    __sh_putf("%s", __tw_5);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    ...
    __sh_lineno=73;
    {
    char __tw_6[1024]; snprintf(__tw_6, sizeof(__tw_6), "info=%s", num);
    __sh_putf("%s", __tw_6);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    ...
    __sh_lineno=74;
    {
    char __tw_7[1024]; snprintf(__tw_7, sizeof(__tw_7), "ok=%s", num);
    __sh_putf("%s", __tw_7);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    ...
    fflush(stdout); dup2(__psi,0); close(__psi); clearerr(stdin); waitpid(__ppid,NULL,0);
    }
