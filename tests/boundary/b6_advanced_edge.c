...
int main(int _argc, char **_argv){
    ...
    strncpy(__sh_script_name, "b6_advanced_edge.sh", 65530);

    __sh_lineno=5;
    {
    char __tw_0[1024]; snprintf(__tw_0, sizeof(__tw_0), "test1: %s", __sh_cmd_output("echo \"inner quoted\""));
    __sh_putf("%s", __tw_0);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=8;
    {
    char __tw_1[1024]; snprintf(__tw_1, sizeof(__tw_1), "test2: %s", __sh_cmd_output("echo a | tr a-z A-Z"));
    __sh_putf("%s", __tw_1);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=11;
    strncpy(s,"a:b:c",sizeof(s)-1);
    __sh_lineno=12;
    {
    char __tw_2[1024]; snprintf(__tw_2, sizeof(__tw_2), "test3: %s", __sh_replace(s,":","_",1,0,0));
    __sh_putf("%s", __tw_2);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=15;
    {
    char __tw_3[1024]; snprintf(__tw_3, sizeof(__tw_3), "test4: %s", ((__sh_getenv("UNSET"))[0]?(__sh_getenv("UNSET")):((__sh_getenv("DEFAULT"))[0]?(__sh_getenv("DEFAULT")):"fallback")));
    __sh_putf("%s", __tw_3);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=18;
    {
    char __tw_4[1024]; snprintf(__tw_4, sizeof(__tw_4), "test5: %s literal vs %s expanded", (__sh_getenv("HOME")), (__sh_getenv("HOME")));
    __sh_putf("%s", __tw_4);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=21;
    ...
    {
    int __an21=0;
    __arr_arr[__an21++]=strdup("x");
    __arr_arr[__an21++]=strdup("y");
    __arr_arr[__an21++]=strdup("z");
    __arr_arr[__an21]=NULL;
    }
    __sh_lineno=22;
    strncpy(ref,"arr",sizeof(ref)-1);
    __sh_lineno=23;
    {
    char __tw_5[1024]; snprintf(__tw_5, sizeof(__tw_5), "test6: %s", (__sh_getenv("!ref")));
    __sh_putf("%s", __tw_5);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=26;
    {
    char __tw_6[1024]; snprintf(__tw_6, sizeof(__tw_6), "test7: %d=%d=%d", __exit_status, (int)getpid(), __sh_last_bg_pid);
    __sh_putf("%s", __tw_6);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=32;
    {
    int __sfd_7_0=dup(0);
    { FILE *__hf=tmpfile(); if(__hf){ char __hdb[16384]; snprintf(__hdb,sizeof(__hdb),"test8: PID=%d line=%s\ntest8b: %s\n",(int)getpid(),(__sh_getenv("LINENO")),__sh_cmd_output("echo nested")); fputs(__hdb,__hf); fflush(__hf); int __hfd=fileno(__hf); lseek(__hfd,0,SEEK_SET); dup2(__hfd,0); clearerr(stdin); } }
    __b_cat(NULL,0);
    fflush(NULL); dup2(__sfd_7_0,0); close(__sfd_7_0);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=35;
    x=10;
    ...
    __sh_lineno=37;
    {
    char __tw_7[1024]; snprintf(__tw_7, sizeof(__tw_7), "test9: %lld", (long long)(x + y));
    __sh_putf("%s", __tw_7);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=42;
    strncpy(multi,"line1\nline2\nline3",sizeof(multi)-1);
    __sh_lineno=43;
    {
    char __tw_8[1024]; snprintf(__tw_8, sizeof(__tw_8), "test10: %d", (int)strlen(multi));
    __sh_putf("%s", __tw_8);
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    __sh_lineno=46;
    strncpy(s2,"test_123",sizeof(s2)-1);
    __sh_lineno=47;
    if(((__sh_regex(s2,"^[a-z]+_[0-9]+$")))){
    __sh_lineno=48;
    {
    __sh_puts("test11: match");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
    __sh_lineno=52;
    strncpy(val,"hello.tar.gz",sizeof(val)-1);
    __sh_lineno=53;
    if((fnmatch("*.tar.gz",__sh_fmt("%s",val),0)==0)){
    __sh_lineno=54;
    {
    __sh_puts("test12: compressed");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    } else if((fnmatch("*.tar",__sh_fmt("%s",val),0)==0)){
    __sh_lineno=55;
    {
    __sh_puts("test12: tar only");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    } else {
    __sh_lineno=56;
    {
    __sh_puts("test12: unknown");
    }
    if(__sh_set_e && __exit_status!=0) exit(__exit_status);
    }
