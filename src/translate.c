...
char *translate_brace_expansion(const char *body){
    ...
    /* ${!var} — indirect, or ${!arr[@]} — array indices */
    if(*p=='!' && *(p+1)!='}'){
        p++;
        while(*p && *p!='}' && *p!='[') name[j++]=*p++;
        name[j]=0;
        char r[2048];
        if(!strcmp(name,"#")){
        ...
        if(*p=='['){
            ...
            idx[k]=0; if(*p) p++;
            if(!strcmp(idx,"@")||!strcmp(idx,"*")){
                snprintf(r,sizeof(r),"__sh_arr_indices(__arr_%s)",safe_cname(name));
                return xstrdup(r);
            }
        }
    ...
    if(*p=='['){
        ...
        if(!strcmp(idx,"@")||!strcmp(idx,"*")){
            ...
            if(*p==':'){
                ...
                /* ${arr[@]#pattern} — strip shortest prefix from each element */
                p++; char pat[256]; int k=0;
                while(*p && *p!='}') pat[k++]=*p++;
                pat[k]=0; if(*p) p++;
                snprintf(r,sizeof(r),"__sh_arr_strip_prefix_all(__arr_%s,\"%s\",0)",safe_cname(name),pat);
            } else if(*p=='%'){
                /* ${arr[@]%%pattern} or ${arr[@]%pattern} — strip suffix from each element */
                int greedy=(*(p+1)=='%'); if(greedy) p++;
                p++; char pat[256]; int k=0;
                while(*p && *p!='}') pat[k++]=*p++;
                pat[k]=0; if(*p) p++;
                snprintf(r,sizeof(r),"__sh_arr_strip_suffix_all(__arr_%s,\"%s\",%d)",safe_cname(name),pat,greedy);
            } else if(*p=='^' || *p==','){
                /* ${arr[@]^^} / ${arr[@],,} — upper/lower each element */
                int upper=(*p=='^'); int all=(*(p+1)==*p);
                p += all ? 2 : 1;
                if(*p=='}') p++;
                snprintf(r,sizeof(r),"__sh_arr_case_all(__arr_%s,%d)",safe_cname(name),upper?(all?2:1):(all?-2:-1));
            } else if(*p=='/'){
                /* ${arr[@]/old/new} or ${arr[@]//old/new} — replace in each element */
                int global=(*(p+1)=='/'); if(global) p++;
                p++; char old[128]; int ok=0;
                while(*p && *p!='/' && *p!='}' && ok<(int)sizeof(old)-1) old[ok++]=*p++;
                old[ok]=0;
                char newp[128]="";
                if(*p=='/'){ p++; int nl=0; while(*p&&*p!='}'&&nl<(int)sizeof(newp)-1) newp[nl++]=*p++; newp[nl]=0; }
                if(*p=='}') p++;
                snprintf(r,sizeof(r),"__sh_arr_replace_all(__arr_%s,\"%s\",\"%s\",%d)",safe_cname(name),old,newp,global);
            } else {
                if(*p=='}') p++;
            ...
            for(const char *q=idx;*q;q++){
                if(!isdigit((unsigned char)*q) && *q!='+' && *q!='-' && *q!='$' && *q!='(' && *q!=')' && *q!='_' && !isalpha((unsigned char)*q)){ is_numeric=0; break; }
            }
            /* Only treat as numeric if it's pure digits, or starts with $var/$(( ))
             * and the array is NOT an associative array (declare -A) */
            if(!(idx[0]=='$' || (idx[0]>='0'&&idx[0]<='9') || idx[0]=='-'||idx[0]=='+')) is_numeric=0;
            if(is_numeric && idx[0]=='$' && get_var_kind(name)==V_ASSOC) is_numeric=0;
            if(is_numeric && idx[0]){
                /* If idx starts with $, translate to C expression first */
                if(idx[0]=='$'){
                    ...
                    if(e && e[0]){
                        ...
                        if(idx[0]=='-'||(idx[0]=='$'&&e[0]=='-')){
                            snprintf(r,sizeof(r),
                                "((__sh_arr_count(__arr_%s)+(%s)>=0)"
                                "?(__arr_%s[__sh_arr_count(__arr_%s)+(%s)]?__arr_%s[__sh_arr_count(__arr_%s)+(%s)]:\"\")"
                                ":(fprintf(stderr,\"%%s: line %%d: %s: bad array subscript\\n\","
                                "__sh_script_name[0]?__sh_script_name:\"shell2c\",__sh_lineno),\"\"))",
                                safe_cname(name),e,
                                safe_cname(name),safe_cname(name),e,
                                safe_cname(name),safe_cname(name),e,
                                name);
                        } else {
                            snprintf(r,sizeof(r),"(__arr_%s[atoi(%s)]?__arr_%s[atoi(%s)]:\"\")",
                                safe_cname(name),e,safe_cname(name),e);
                        }
                        free(e);
                        return xstrdup(r);
                    }
                ...
                /* Check for negative index: ${arr[-1]} → arr[arr_count-1] */
                if(idx[0]=='-' && isdigit((unsigned char)idx[1])){
                    /* Negative index with bounds check: if count+idx<0, "bad array subscript" */
                    snprintf(r,sizeof(r),
                        "((__sh_arr_count(__arr_%s)+(%s)>=0)"
                        "?(__arr_%s[__sh_arr_count(__arr_%s)+(%s)]?__arr_%s[__sh_arr_count(__arr_%s)+(%s)]:\"\")"
                        ":(fprintf(stderr,\"%%s: line %%d: %s: bad array subscript\\n\","
                        "__sh_script_name[0]?__sh_script_name:\"shell2c\",__sh_lineno),\"\"))",
                        safe_cname(name),idx,
                        safe_cname(name),safe_cname(name),idx,
                        safe_cname(name),safe_cname(name),idx,
                        name);
                } else {
                    snprintf(r,sizeof(r),"(__arr_%s[%s]?__arr_%s[%s]:\"\")",safe_cname(name),idx,safe_cname(name),idx);
                }
            } else {
    ...
    if(*p==':' && (*(p+1)=='-'||*(p+1)=='='||*(p+1)=='+'||*(p+1)=='?')){
        ...
        if(vk == V_INT){
            ...
            if(def_expanded){
                ...
                /* Strip surrounding quotes if present (e.g. ${x:-"default"}) */
                char *def_start=def; int def_len=strlen(def);
                if(def_len>=2 && ((def[0]=='"'&&def[def_len-1]=='"') || (def[0]=='\''&&def[def_len-1]=='\''))){
                    def_start=def+1; def_len-=2;
                }
                snprintf(def_buf,sizeof(def_buf),"\"%.*s\"",def_len,def_start);
                def_val = def_buf;
            }
            switch(op){
                ...
                case '?': {
                    /* Escape def for C string literal */
                    char esc_def[512]; int ei=0;
                    for(const char *dp=def;*dp&&ei<(int)sizeof(esc_def)-4;dp++){
                        if(*dp=='"'){ esc_def[ei++]='\\'; esc_def[ei++]='"'; }
                        else if(*dp=='\\'){ esc_def[ei++]='\\'; esc_def[ei++]='\\'; }
                        else if(*dp=='\n'){ esc_def[ei++]='\\'; esc_def[ei++]='n'; }
                        else esc_def[ei++]=*dp;
                    }
                    esc_def[ei]=0;
                    snprintf(r,sizeof(r),"(%s[0]?%s:(fprintf(stderr,\"%%s: line %%d: %s: %s\\n\",__sh_script_name[0]?__sh_script_name:\"shell2c\",__sh_lineno),exit(1),\"\"))",cn,cn,name,esc_def);
                    break;
                }
    ...
    if(*p=='#'){
        ...
        while(*p && !(*p=='}' && bd==0)){
            ...
            if(k<(int)sizeof(pat)-1) pat[k++]=*p;
            p++;
        }
        pat[k]=0; if(*p) p++;
        char r[2048];
        /* Check if pattern contains ${...} — needs runtime evaluation */
        if(strstr(pat,"${")){
            /* Strip surrounding double quotes if present */
            char ipat[256]; strncpy(ipat,pat,sizeof(ipat)-1); ipat[sizeof(ipat)-1]=0;
            if(ipat[0]=='"'){
                int pl=strlen(ipat);
                if(pl>1 && ipat[pl-1]=='"'){ ipat[pl-1]=0; memmove(ipat,ipat+1,strlen(ipat)); }
            }
            /* If entire pattern is ${...} — translate directly */
            int il=strlen(ipat);
            if(ipat[0]=='$' && ipat[1]=='{' && il>3 && ipat[il-1]=='}'){
                char ibody[256]; int ilen=il-3;
                strncpy(ibody,ipat+2,ilen); ibody[ilen]=0;
                char *e2=translate_brace_expansion(ibody);
                if(e2){
                    snprintf(r,sizeof(r),"__sh_strip_prefix(%s,%s,%d)",safe_cname(name),e2,greedy);
                    free(e2);
                    return xstrdup(r);
                }
            }
            /* Mixed pattern — use expand_string + __sh_fmt */
            ExpandResult er; expand_string(pat,&er);
            if(er.nargs>0){
                char args_buf[1024]; int ai=0;
                for(int i=0;i<er.nargs && ai<(int)sizeof(args_buf)-16;i++){
                    if(i>0) args_buf[ai++]=',';
                    int al=strlen(er.args[i]);
                    if(ai+al<(int)sizeof(args_buf)-8){ memcpy(args_buf+ai,er.args[i],al); ai+=al; }
                }
                args_buf[ai]=0;
                /* Escape format string for C string literal */
                char esc_fmt[512]; int ei=0;
                for(const char *fp=er.fmt;*fp && ei<(int)sizeof(esc_fmt)-4;fp++){
                    if(*fp=='"'||*fp=='\\') esc_fmt[ei++]='\\';
                    esc_fmt[ei++]=*fp;
                }
                esc_fmt[ei]=0;
                snprintf(r,sizeof(r),"__sh_strip_prefix(%s,__sh_fmt(\"%s\",%s),%d)",
                    safe_cname(name),esc_fmt,args_buf,greedy);
                expand_free(&er);
                return xstrdup(r);
            }
            expand_free(&er);
    ...
    if(*p=='%'){
        ...
        if(strstr(pat,"${")){
            ...
            if(ipat[0]=='$' && ipat[1]=='{' && il>3 && ipat[il-1]=='}'){
                ...
                char *e2=translate_brace_expansion(ibody);
                if(e2){
                    snprintf(r,sizeof(r),"__sh_strip_suffix(%s,%s,%d)",safe_cname(name),e2,greedy);
                    free(e2);
                    return xstrdup(r);
                }
            }
            ...
            if(er.nargs>0){
                ...
                esc_fmt[ei]=0;
                snprintf(r,sizeof(r),"__sh_strip_suffix(%s,__sh_fmt(\"%s\",%s),%d)",
                    safe_cname(name),esc_fmt,args_buf,greedy);
                expand_free(&er);
                return xstrdup(r);
            }
    ...
    if(*p=='/'){
        ...
        while(*p && *p!='}' && k<(int)sizeof(old)-1){
            if(*p=='\\' && *(p+1)=='/'){ old[k++]='/'; p+=2; continue; } /* \/ → / */
            if(*p=='/' && k>0) break; /* unescaped / is separator */
            if(*p=='\\' && *(p+1)){ old[k++]=*p++; old[k++]=*p++; continue; } /* keep other escapes */
            if(*p=='/') break;
            old[k++]=*p++;
        }
        old[k]=0;
        char newp[128]="";
        if(*p=='/'){ p++; int l=0; while(*p&&*p!='}'&&l<(int)sizeof(newp)-1){
            if(*p=='\\' && *(p+1)=='/'){ newp[l++]='/'; p+=2; continue; }
            if(*p=='\\' && *(p+1)){ newp[l++]=*p++; newp[l++]=*p++; continue; }
            newp[l++]=*p++;
        } newp[l]=0; }
        if(*p=='}') p++;
        char r[2048];
        ...
        /* Escape old and newp for C string literal */
        char esc_old[256]; int eo=0;
        for(int i=0;old[i]&&eo<(int)sizeof(esc_old)-2;i++){
            if(old[i]=='"'||old[i]=='\\') esc_old[eo++]='\\';
            esc_old[eo++]=old[i];
        }
        esc_old[eo]=0;
        char esc_new[256]; int en=0;
        for(int i=0;newp[i]&&en<(int)sizeof(esc_new)-2;i++){
            if(newp[i]=='"'||newp[i]=='\\') esc_new[en++]='\\';
            esc_new[en++]=newp[i];
        }
        esc_new[en]=0;
        snprintf(r,sizeof(r),"__sh_replace(%s,\"%s\",\"%s\",%d,%d,%d)",
                 cn,esc_old,esc_new,global,anchor_start,anchor_end);
        return xstrdup(r);
    }
    /* ${var^^}  ${var,,} — case conversion (all chars) */
    /* ${var^}   ${var,}  — case conversion (first char only) */
    if(*p=='^'){
        ...
        if(all)
            ...
            snprintf(r,sizeof(r),"__sh_upper_first(%s)",safe_cname(name));
        return xstrdup(r);
    }
    if(*p==','){
        int all=(*(p+1)==','); if(all) p++;
        ...
        if(all)
            ...
            snprintf(r,sizeof(r),"__sh_lower_first(%s)",safe_cname(name));
        return xstrdup(r);
    }
    /* plain ${var} */
    if(*p=='}') p++;
...
char *expand_cmd_subst(const char *cmd){
    ...
    while(*p && fi<(int)sizeof(fmt)-16){
        if(*p=='$'){
            ...
            if(*p=='"'||*p=='\\') fmt[fi++]='\\';
            if(*p=='\n'){ fmt[fi++]='\\'; fmt[fi++]='n'; p++; continue; }
            if(*p=='\t'){ fmt[fi++]='\\'; fmt[fi++]='t'; p++; continue; }
            fmt[fi++]=*p++;
        }
    }
    fmt[fi]=0; args[ai]=0;
...
char *translate_arith(const char *expr){
    ...
    while(*s && q<(int)sizeof(buf)-8){
        if(isalpha((unsigned char)*s)||*s=='_'){
            ...
            /* check for compound arithmetic assignment: += -= *= /= %=
             * In ((...)) arithmetic context, these are always arithmetic operations */
            if((*np=='+'&&*(np+1)=='='&&*(np+2)!='=') ||
               (*np=='-'&&*(np+1)=='='&&*(np+2)!='=') ||
               (*np=='*'&&*(np+1)=='='&&*(np+2)!='=') ||
               (*np=='/'&&*(np+1)=='='&&*(np+2)!='=') ||
               (*np=='%'&&*(np+1)=='='&&*(np+2)!='=')){
                /* Arithmetic compound assignment: x += 5 → x += 5 (C operator) */
                if(vk==V_STR){
                    /* String var: convert to int for arithmetic */
                    q+=snprintf(buf+q,sizeof(buf)-q,"atoi(%s)",cn);
                    buf[q++]=*np;     /* + - * / % */
                    buf[q++]='=';
                } else {
            ...
            /* Detect division/modulo by zero — emit runtime check + fallback */
            if(*s=='/'||*s=='%'){
                const char *np=s+1;
                while(*np==' '||*np=='\t') np++;
                if(*np=='0' && !isdigit((unsigned char)*(np+1)) && *(np+1)!='x' && *(np+1)!='X'){
                    /* /0 or %0 — emit runtime error + safe fallback */
                    /* Build expression string for error message */
                    char div_expr[64]; int di2=0;
                    /* Look back to find the dividend */
                    int bs2=q; while(bs2>0 && (isdigit((unsigned char)buf[bs2-1])||buf[bs2-1]==' ')) bs2--;
                    /* copy dividend, skipping trailing spaces */
                    int be2=bs2;
                    while(be2<q){
                        int end2=be2; while(end2<q && buf[end2]==' ') end2++; /* skip leading spaces */
                        if(end2>=q) break;
                        while(end2<q && buf[end2]!=' ') div_expr[di2++]=buf[end2++];
                        be2=end2;
                        if(be2<q && buf[be2]==' ') { /* skip one space between tokens */ }
                        be2++;
                    }
                    { int sp_b=(s>expr&&(s[-1]==' '||s[-1]=='\t'));
                      int sp_a=(s[1]==' '||s[1]=='\t');
                      if(sp_b) div_expr[di2++]=' ';
                      div_expr[di2++]=*s;
                      if(sp_a) div_expr[di2++]=' ';
                      div_expr[di2++]='0'; div_expr[di2]=0; }
                    q=bs2; /* remove dividend from buf — it'll be in the call */
                    q+=snprintf(buf+q,sizeof(buf)-q,"(__sh_div_zero(\"%s\"),1)",div_expr);
                    s=np+1; continue;
                } else if(isalpha((unsigned char)*np)||*np=='_'){
                    /* Division/modulo by variable — use __sh_safe_div/mod to
                     * catch runtime zero divisor (avoids SIGFPE crash) */
                    ...
                    div_var[dvi]=0;
                    /* Look back for dividend expression in buf */
                    int bs2=q;
                    while(bs2>0 && buf[bs2-1]==' ') bs2--;
                    int ds2=bs2;
                    if(bs2>0 && buf[bs2-1]==')'){
                        int pd=1; int pp=bs2-1;
                        while(pp>0 && pd>0){ pp--; if(buf[pp]==')') pd++; else if(buf[pp]=='(') pd--; }
                        while(pp>0 && (isalnum((unsigned char)buf[pp-1])||buf[pp-1]=='_')) pp--;
                        ds2=pp;
                    } else {
                        while(ds2>0 && (isalnum((unsigned char)buf[ds2-1])||buf[ds2-1]=='_')) ds2--;
                    }
                    char dividend[256]; int dl=0;
                    for(int i=ds2;i<bs2&&dl<255;i++) dividend[dl++]=buf[i];
                    dividend[dl]=0;
                    ...
                    q=ds2;
                    if(*s=='/')
                        q+=snprintf(buf+q,sizeof(buf)-q,"__sh_safe_div(%s,%s,\"%s/%s\",\"%s\")",dividend,div_expr_c,dividend,div_var,div_var);
                    else
                        q+=snprintf(buf+q,sizeof(buf)-q,"__sh_safe_mod(%s,%s,\"%s%%%s\",\"%s\")",dividend,div_expr_c,dividend,div_var,div_var);
                    s=dp; continue;
                }
            }
            /* ** → power operator */
            if(*s=='*'&&*(s+1)=='*'){
                /* Find base: last expression in buf.
                 * Base can be: identifier, number, or (...) balanced parens */
                int bs=q;
                /* Skip trailing whitespace */
                while(bs>0 && (buf[bs-1]==' '||buf[bs-1]=='\t')) bs--;
                if(bs>0 && buf[bs-1]==')'){
                    /* Base ends with ) — scan back for matching ( */
                    int pd=1; bs--;
                    while(bs>0 && pd>0){ bs--; if(buf[bs]==')') pd++; else if(buf[bs]=='(') pd--; }
                    /* bs now points to ( — include the ( */
                } else {
                    /* Base is identifier or number — scan back alnum/_ */
                ...
                if(bs<q){
                    ...
                    q=bs;
                    const char *fn="(long long)__sh_pow("; while(*fn) buf[q++]=*fn++;
    ...
    if(strncmp(tok,"$((",3)==0){
        ...
        while(*s){
            ...
            if(*s=='$'){
                ...
                /* But NOT $((...)) which is arithmetic */
                if(*s=='(' && *(s+1)!='('){
                    s++; /* skip ( */
                    char cmd[1024]; int ci=0; int d=1;
                    while(*s && d && ci<(int)sizeof(cmd)-1){
                        if(*s=='(') d++;
                        else if(*s==')'){ d--; if(d==0) break; }
                        cmd[ci++]=*s++;
                    }
                    cmd[ci]=0; if(*s==')') s++;
                    /* Check if cmd is a user function */
                    ...
                    if(is_user_func(fname)){
                        ...
                        while(*ap && nargs<9){
                            ...
                            if(*ap=='"'){ ap++; while(*ap&&*ap!='"'&&al<255) arg[al++]=*ap++; if(*ap=='"')ap++; }
                            else { while(*ap&&*ap!=' '&&*ap!='\t'&&al<255) arg[al++]=*ap++; }
                            arg[al]=0;
                            if(al>0){
                                buf[q++]=','; buf[q++]='"';
                                for(int ai=0;ai<al&&q<(int)sizeof(buf)-4;ai++) buf[q++]=arg[ai];
                                buf[q++]='"'; nargs++;
                            }
                        }
                        buf[q++]=','; buf[q++]='N'; buf[q++]='U'; buf[q++]='L'; buf[q++]='L';
                        buf[q++]=')'; buf[q++]=')';
                    } else {
                        /* External command */
                        const char *pre="atoi(__sh_cmd_output(\"";
                        while(*pre) buf[q++]=*pre++;
                        /* escape cmd for C string */
                        for(int ei=0;cmd[ei]&&q<(int)sizeof(buf)-4;ei++){
                            if(cmd[ei]=='"') buf[q++]='\\';
                            if(cmd[ei]=='\\') buf[q++]='\\';
                            buf[q++]=cmd[ei];
                        }
                ...
                if(*s=='{'){
                    s++;
                    /* Use translate_brace_expansion for ${...} inside $(( )) */
                    char brace_body[256]; int bj=0;
                    while(*s && *s!='}' && bj<(int)sizeof(brace_body)-1) brace_body[bj++]=*s++;
                    brace_body[bj]=0; if(*s) s++;
                    char *e=translate_brace_expansion(brace_body);
                    int el=(int)strlen(e);
                    if(q+el<(int)sizeof(buf)-2){ memcpy(buf+q,e,el); q+=el; }
                    free(e);
                    continue;
                ...
                /* check for += compound assignment (bash string append) */
                if(*s=='+'&&*(s+1)=='='&&*(s+2)!='='){
                    /* x+=val → strncat(x, val, sizeof(x)-strlen(x)-1) */
                    s+=2;
                    while(*s==' '||*s=='\t') s++;
                    if(*s=='"'){
                        s++;
                        char rhs[256]; int ri=0;
                        while(*s && *s!='"' && ri<(int)sizeof(rhs)-1) rhs[ri++]=*s++;
                        if(*s=='"') s++;
                        rhs[ri]=0;
                        q+=snprintf(buf+q,sizeof(buf)-q,"strncat(%s,\"%s\",sizeof(%s)-strlen(%s)-1)",cn,rhs,cn,cn);
                    } else {
                        char rhs[256]; int ri=0;
                        while(*s && *s!=';' && *s!='"' && ri<(int)sizeof(rhs)-1) rhs[ri++]=*s++;
                        rhs[ri]=0;
                        int is_num=1;
                        for(int i=0;i<ri;i++){
                            if(!isdigit((unsigned char)rhs[i]) && rhs[i]!='-'){ is_num=0; break; }
                        }
                        if(is_num){
                            q+=snprintf(buf+q,sizeof(buf)-q,"strncat(%s,\"%s\",sizeof(%s)-strlen(%s)-1)",cn,rhs,cn,cn);
                        } else {
                            const char *cn2=safe_cname(rhs);
                            q+=snprintf(buf+q,sizeof(buf)-q,"strncat(%s,%s,sizeof(%s)-strlen(%s)-1)",cn,cn2,cn,cn);
                        }
                    }
                    continue;
                }
                ...
                /* Detect division/modulo by zero — emit runtime error + safe fallback */
                if(*s=='/'||*s=='%'){
                    const char *np=s+1;
                    while(*np==' '||*np=='\t') np++;
                    if(*np=='0' && !isdigit((unsigned char)*(np+1)) && *(np+1)!='x' && *(np+1)!='X'){
                        ...
                        char div_expr[64]; int di2=0;
                        int bs2=q; while(bs2>0 && (isdigit((unsigned char)buf[bs2-1])||buf[bs2-1]==' ')) bs2--;
                        int be2=bs2; while(be2<q) div_expr[di2++]=buf[be2++];
                        while(di2>0 && div_expr[di2-1]==' ') di2--;  /* strip trailing spaces */
                        { int sp_b=(s>tok&&(s[-1]==' '||s[-1]=='\t'));
                          int sp_a=(s[1]==' '||s[1]=='\t');
                          if(sp_b) div_expr[di2++]=' ';
                          div_expr[di2++]=*s;
                          if(sp_a) div_expr[di2++]=' ';
                        ...
                        int bs2=q;
                        while(bs2>0 && buf[bs2-1]==' ') bs2--;
                        int ds2=bs2; /* dividend start */
                        if(bs2>0 && buf[bs2-1]==')'){
                            /* Match parens to find function call start (e.g. atoi(...)) */
                            int pd=1; int pp=bs2-1;
                            while(pp>0 && pd>0){ pp--; if(buf[pp]==')') pd++; else if(buf[pp]=='(') pd--; }
                            /* pp points to (, look for function name before it */
                            while(pp>0 && (isalnum((unsigned char)buf[pp-1])||buf[pp-1]=='_')) pp--;
                            ds2=pp;
                        } else {
                            while(ds2>0 && (isalnum((unsigned char)buf[ds2-1])||buf[ds2-1]=='_')) ds2--;
                        }
                        /* Extract dividend expression */
                        char dividend[256]; int dl=0;
                        for(int i=ds2;i<bs2&&dl<255;i++) dividend[dl++]=buf[i];
                        dividend[dl]=0;
                        /* Build divisor expression (handle string vars) */
                        const char *cn2=safe_cname(div_var);
                        VarKind dvk=get_var_kind(div_var);
                        char div_expr_c[256];
                        if(dvk==V_STR) snprintf(div_expr_c,sizeof(div_expr_c),"atoi(%s)",cn2);
                        else snprintf(div_expr_c,sizeof(div_expr_c),"%s",cn2);
                        /* Replace dividend in buf with safe call */
                        q=ds2;
                        if(*s=='/')
                            q+=snprintf(buf+q,sizeof(buf)-q,"__sh_safe_div(%s,%s)",dividend,div_expr_c);
                        else
                            q+=snprintf(buf+q,sizeof(buf)-q,"__sh_safe_mod(%s,%s)",dividend,div_expr_c);
                        s=dp; continue;
                    }
                }
                buf[q++]=*s++;
    ...
    if(tok[0]=='$'){
        ...
        if(!strcmp(name,"RANDOM")||!strcmp(name,"SECONDS")||!strcmp(name,"LINENO")||
           ...
           !strcmp(name,"OSTYPE")||!strcmp(name,"MACHTYPE")){
            char r[2048]; snprintf(r,sizeof(r),"__sh_special_var(\"%s\")",name);
            return xstrdup(r);
        }
