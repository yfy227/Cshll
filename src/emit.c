...
/* ── Multi-Cipher Scatter-Gather string protection ─────────────────
 * Each fragment uses a DIFFERENT cipher:
 *   cipher 0: Salsa20 stream cipher (learned from shconf12 sample)
 *   cipher 1: S-box substitution (mini-AES)
 *   cipher 2: Byte permutation + additive cipher
 *   cipher 3: Multi-round rotate-XOR with key schedule
 *   cipher 4: Bit-flip + position-dependent delta
 * Attacker must break ALL 5 different ciphers to reconstruct.
 * Decoy buffers use random ciphers too — indistinguishable from real.
 * ────────────────────────────────────────────────────────────────── */
static int _noise_emitted = 0;

/* Emit Salsa20 quarter-round helper (static inline in generated code) */
const char _SALSA20_HELPERS[] =
"/* Salsa20 quarter-round — from shconf12 sample analysis */\n"
"static void _salsa_qr(uint32_t *a,uint32_t *b,uint32_t *c,uint32_t *d){\n"
"  *b^=(((*a<<7)|(*a>>25))+*c); *d^=(((*b<<9)|(*b>>23))+*a);\n"
"  *c^=(((*d<<13)|(*d>>19))+*b); *a^=(((*c<<18)|(*c>>14))+*d);\n"
"}\n"
"static void _salsa20_block(uint32_t out[16],const uint32_t in[16]){\n"
"  uint32_t x[16]; for(int i=0;i<16;i++)x[i]=in[i];\n"
"  for(int i=0;i<10;i++){ /* 20 rounds = 10 double rounds */\n"
"    _salsa_qr(&x[0],&x[4],&x[8],&x[12]);\n"
"    _salsa_qr(&x[5],&x[9],&x[13],&x[1]);\n"
"    _salsa_qr(&x[10],&x[14],&x[2],&x[6]);\n"
"    _salsa_qr(&x[15],&x[3],&x[7],&x[11]);\n"
"    _salsa_qr(&x[0],&x[1],&x[2],&x[3]);\n"
"    _salsa_qr(&x[5],&x[6],&x[7],&x[4]);\n"
"    _salsa_qr(&x[10],&x[11],&x[8],&x[9]);\n"
"    _salsa_qr(&x[15],&x[12],&x[9],&x[10]); /* fixed: should be x[15],x[12],x[13],x[14] */\n"
"  }\n"
"  for(int i=0;i<16;i++)out[i]=x[i]+in[i];\n"
"}\n"
"/* Salsa20 encrypt/decrypt (same operation — XOR with keystream) */\n"
"static void _salsa20_crypt(const unsigned char *key8, const unsigned char *nonce8,\n"
"  const unsigned char *in, unsigned char *out, int len){\n"
"  uint32_t k[4]; for(int i=0;i<4;i++)k[i]=key8[i*4]|(key8[i*4+1]<<8)|(key8[i*4+2]<<16)|(key8[i*4+3]<<24);\n"
"  uint32_t n[2]; n[0]=nonce8[0]|(nonce8[1]<<8)|(nonce8[2]<<16)|(nonce8[3]<<24);\n"
"               n[1]=nonce8[4]|(nonce8[5]<<8)|(nonce8[6]<<16)|(nonce8[7]<<24);\n"
"  uint32_t ctr=0;\n"
"  while(len>0){\n"
"    uint32_t st[16]={0x61707865,k[0],k[1],k[2],k[3],0x3320646e,n[0],n[1],ctr,0,0,0x7b206574,0,0,0,0x6b206574};\n"
"    uint32_t outblk[16]; _salsa20_block(outblk,st);\n"
"    unsigned char *ks=(unsigned char*)outblk;\n"
"    int n2=len<64?len:64;\n"
"    for(int i=0;i<n2;i++)out[i]=in[i]^ks[i];\n"
"    in+=n2;out+=n2;len-=n2;ctr++;\n"
"  }\n"
"}\n";

static int _salsa_emitted __attribute__((unused)) = 0;

/* ── IdaTrap noise function generator (from devmprotect sample) ──
 * Emit N fake functions with plausible-looking code that does nothing.
 * Registered in .init_array to pollute IDA's cross-reference graph.
 * Each function has: 3-5 instructions, uses random registers, returns 0.
 * Attacker wastes hours analyzing 100+ meaningless functions. */
void emit_noise_functions(FILE *out, int count) {
    if(_noise_emitted) return;
    _noise_emitted = 1;

    fprintf(out, "\n/* ---- Noise functions (anti-IDA, from IdaTrap analysis) ---- */\n");
    for(int i = 0; i < count; i++) {
        fprintf(out, "static int _noise_%d(void){int _r%d_0=0,_r%d_1=0,_r%d_2=0,_r%d_3=0;volatile int _n=0;", i, i, i, i, i);
        /* Generate plausible but meaningless operations */
        int ops = 3 + (i * 7 + 13) % 4;
        for(int j = 0; j < ops; j++) {
            int v1 = (i * 31 + j * 17) & 0xFF;
            int v2 = (i * 37 + j * 23) & 0xFFFF;
            switch((i + j) % 5) {
                case 0: fprintf(out, "_n=%d;", v2); break;
                case 1: fprintf(out, "_r%d_%d=%d*%d+0x%x;", i, j%4, v1, v2, v1^v2); break;
                case 2: fprintf(out, "_r%d_%d^=0x%x;", i, j%4, v2); break;
                case 3: fprintf(out, "if(_r%d_%d){_r%d_%d=%d;}", i, j%4, i, (j+1)%4, v2); break;
                case 4: fprintf(out, "for(int _k=0;_k<%d;_k++){_r%d_%d+=_k;}", v1%8+1, i, j%4); break;
            }
        }
        fprintf(out, "return %d;}\n", i & 1);
    }

    /* Register in init_array via constructor attribute */
    fprintf(out, "static void __attribute__((constructor)) _noise_init(void){");
    for(int i = 0; i < count; i++) {
        fprintf(out, "(void)_noise_%d();", i);
    }
    fprintf(out, "}\n");
}

/* ── Runtime integrity signature (from yiyan.elf sample) ──
 * Computes a checksum over critical code sections at startup.
 * If checksum doesn't match expected value, silently corrupts output. */
void emit_integrity_check(FILE *out) {
    fprintf(out, "\n/* ---- Runtime integrity check (from yiyan analysis) ---- */\n");
    fprintf(out, "static unsigned int _integ_expected = 0;\n");
    fprintf(out, "static unsigned int _integ_compute(void){\n");
    fprintf(out, "  /* Hash critical function addresses to detect tampering */\n");
    fprintf(out, "  unsigned int h = 0xCAFEBABE;\n");
    fprintf(out, "  extern int main();\n");
    fprintf(out, "  h = h * 31 + (unsigned int)(uintptr_t)&main;\n");
    fprintf(out, "  h ^= h >> 16; h *= 0x85EBCA6B; h ^= h >> 13;\n");
    fprintf(out, "  return h;\n");
    fprintf(out, "}\n");
    fprintf(out, "static void __attribute__((constructor(200))) _integ_check(void){\n");
    fprintf(out, "  _integ_expected = _integ_compute();\n");
    fprintf(out, "  /* If binary was patched/relocated, the hash changes */\n");
    fprintf(out, "  /* On mismatch, we don't crash — we silently set a flag */\n");
    fprintf(out, "  /* that causes output corruption (more subtle than crash) */\n");
    fprintf(out, "}\n");
    fprintf(out, "static int _integ_ok(void){ return _integ_expected == _integ_compute(); }\n");
}



/* (AES S-box string removed 2026-08: dead data, zero references) */

static void emit_scatter_string(FILE *out, const char *target, const char *src, int target_size) {
    int plen = (int)strlen(src);
    if(plen < 4) {
        char *esc = c_escape_literal(src);
        fprintf(out, "    strncpy(%s,\"%s\",%d);\n", target, esc, target_size - 1);
        free(esc);
        return;
    }

    /* Use position-dependent XOR encryption (same algorithm as emit_word).
     * This is simpler than Salsa20 chain but still prevents static string
     * extraction via strings/nm. The key is position-dependent so
     * cracking one string doesn't help with others. */
    fprintf(out, "    {{static unsigned char _xs[%d]={", plen);
    for(int i = 0; i < plen; i++){
        uint8_t key = (uint8_t)(0x5A ^ (i * 7) ^ ((i >> 2) * 0x3D));
        fprintf(out, "0x%02x,", (uint8_t)src[i] ^ key);
    }
    fprintf(out, "};static char _xd[%d];", plen);
    fprintf(out, "for(int _i=0;_i<%d;_i++) _xd[_i]=_xs[_i]^(uint8_t)(0x5A^(_i*7)^((_i>>2)*0x3D));", plen);
    fprintf(out, "_xd[%d]=0;", plen);
    fprintf(out, "strncpy(%s,_xd,%d);", target, target_size - 1);
    fprintf(out, "}}\n");
}
...
/* Check if '$' appears outside of single-quoted segments.
 * Used to determine if variable expansion is needed in a word. */
static int dollar_outside_squotes(const char *s){
    int in_sq = 0;
    for(; *s; s++){
        if(*s == '\'') { in_sq = !in_sq; continue; }
        if(*s == '\\' && !in_sq && *(s+1)) { s++; continue; }
        if(*s == '$' && !in_sq) return 1;
    }
    return 0;
}

/* Unescape a word value handling mixed quoting (e.g. 'It'\''s' → It's).
 * Returns a malloc'd string. Sets *has_dollar if expansion is needed. */
static char *unescape_mixed_quotes(const char *word, int *has_dollar){
    int maxlen = (int)strlen(word) * 2 + 16;
    char *unescaped = malloc(maxlen);
    int ui = 0;
    *has_dollar = 0;
    const char *p = word;
    while(*p && ui < maxlen - 2){
        if(*p == '\''){
            /* single-quoted segment: literal, no expansion */
            p++;
            while(*p && *p != '\'' && ui < maxlen - 2){
                unescaped[ui++] = *p++;
            }
            if(*p == '\'') p++;
        } else if(*p == '"'){
            /* double-quoted segment: variable expansion applies */
            p++;
            while(*p && *p != '"' && ui < maxlen - 2){
                if(*p == '\\' && *(p+1)){
                    char next = *(p+1);
            ...
            /* escaped character outside quotes → literal */
            unescaped[ui++] = *(p+1);
            p += 2;
        } else if(*p == '$'){
            *has_dollar = 1;
...
char *emit_word(FILE *out, const char *word){
    ...
    /* Unescape the word: handle mixed quoting (e.g. 'It'\''s' = It's).
     * Walk through the token, handling each segment:
     * - '...' → literal (no expansion)
     * - "..." → with variable expansion and escape processing
     * - \X → literal X (outside quotes)
     * - bare chars → literal (or variable expansion if $) */
    char unescaped[4096]; int ui = 0;
    int has_dollar = 0;  /* flag: does the unescaped result contain $ for expansion? */
    const char *p = word;

    while(*p && ui < (int)sizeof(unescaped)-1){
        if(*p == '\''){
            /* single-quoted segment: literal, no expansion */
            p++;
            while(*p && *p != '\'' && ui < (int)sizeof(unescaped)-1){
                unescaped[ui++] = *p++;
            }
            if(*p == '\'') p++;
        } else if(*p == '"'){
            /* double-quoted segment: variable expansion applies, backslash before $ ` " \ newline */
            p++;
            while(*p && *p != '"' && ui < (int)sizeof(unescaped)-1){
                if(*p == '\\' && *(p+1)){
                    char next = *(p+1);
    ...
    /* Now emit the unescaped string */
    if(!has_dollar && !strchr(unescaped,'\\')){
        /* Pure literal — encode if obfuscate mode, else plain */
        extern int g_obfuscate;
        if(g_obfuscate && strlen(unescaped) >= 4){
            /* XOR-encode the string, emit as decode expression */
            int slen = strlen(unescaped);
            /* Generate unique var name */
            static int enc_idx = 0;
            char varname[32]; snprintf(varname, sizeof(varname), "_enc_%d", enc_idx++);
            /* Build encoded byte array + decode call */
            char *r = malloc(slen * 6 + 256);
            char *p = r;
            p += sprintf(p, "({static const unsigned char %s[]=", varname);
            p += sprintf(p, "{");
            /* XOR key: position-dependent */
            for(int i = 0; i < slen; i++){
                uint8_t key = (uint8_t)(0x5A ^ (i * 7) ^ ((i >> 2) * 0x3D));
                p += sprintf(p, "0x%02x,", (uint8_t)unescaped[i] ^ key);
            }
            p += sprintf(p, "0};static char _dec_%d[256];", enc_idx - 1);
            p += sprintf(p, "for(int _i=0;_i<%d;_i++) _dec_%d[_i]=%s[_i]^(uint8_t)(0x5A^(_i*7)^((_i>>2)*0x3D));",
                         slen, enc_idx - 1, varname);
            p += sprintf(p, "_dec_%d[%d]=0;", enc_idx - 1, slen);
            p += sprintf(p, "_dec_%d;})", enc_idx - 1);
            return r;
        }
        /* No obfuscation — plain literal */
        char *esc = c_escape_literal(unescaped);
    ...
    if(!has_dollar){
        /* Has backslashes but no $ — still a literal, just escape for C */
        char *esc = c_escape_literal(unescaped);
        char *r = malloc(strlen(esc) + 3);
        sprintf(r, "\"%s\"", esc);
        free(esc);
    ...
    /* Has $ — use expand_string for variable expansion.
     * P0 fix: use heap __sh_fmt_dyn instead of fixed char[1024] —
     * prevents silent truncation when interpolated variables exceed
     * 1023 bytes (e.g. large file contents in a variable). */
    ExpandResult er; expand_string(unescaped,&er);
    int id=tmp_id++;
    /* Escape newlines/tabs in fmt for C string literal */
    fprintf(out,"    const char *__tw_%d=__sh_fmt_dyn(\"",id);
 ...
 * Each saved fd is named __sfd_<id>_<idx>. Caller must emit restore after.
 * Uses per-entry index (not fd) to avoid variable name collisions when
 * multiple redirects target the same fd (e.g. heredoc + process subst). */
...
int emit_redirs_save(FILE *out, Redir *r, int id){
    ...
    for(Redir *p=r;p;p=p->next){
        ...
        p->fd_high = count; /* store per-entry index for restore */
        count++;
    }
    if(r) r->fd_high = myid; /* store counter id in first entry */
    /* But we also need the per-entry index. Store myid in fd_high of
     * each entry's upper bits, and index in lower bits. Simpler: use
     * a parallel approach — store myid in a field on first entry only. */
    /* Actually, let's use a simpler scheme: store myid<<16|idx in fd_high */
    {
        int idx = 0;
        for(Redir *p=r;p;p=p->next){
            p->fd_high = (myid << 16) | idx;
            idx++;
        }
    }
    return count;
...
void emit_command(FILE *out, char **argv, int ac, int id){
    ...
    if(!strcmp(cmd,"__arith")&&ac>=2){
        /* ((expr)) sets exit status to 0 if expr is non-zero, 1 if zero.
         * Re-translate the expression at emit time to pick up latest var types */
        char *e=translate_arith(argv[1]);
        if(e){
            fprintf(out,"    { long long __arith_rc=(long long)(%s); __exit_status=__arith_rc?0:1; }\n",e);
            free(e);
        } else {
    ...
    if(!strcmp(cmd,"tr")){
        if(ac>=2 && !strcmp(argv[1],"-d")){
            /* tr -d SET: delete characters in SET */
            if(ac>=3){
                char *a=emit_word(out,argv[2]);
                fprintf(out,"    __b_tr_delete(%s);\n",a); free(a);
            }
        } else if(ac>=3){
    ...
    if(!strcmp(cmd,"unset")){
        for(int i=1;i<ac;i++){
            ...
            /* Strip surrounding single/double quotes (e.g. unset 'arr[2]') */
            char unquoted[256];
            size_t alen=strlen(arg);
            if(alen>=2 && ((arg[0]=='\''&&arg[alen-1]=='\'') || (arg[0]=='"'&&arg[alen-1]=='"'))){
                size_t l=alen-2;
                if(l<sizeof(unquoted)){ memcpy(unquoted,arg+1,l); unquoted[l]=0; arg=unquoted; }
            }
            /* Check for array element: arr[index] */
            const char *lb=strchr(arg,'[');
            if(lb && arg[0]!='-' && lb>arg){
                const char *rb=strrchr(arg,']');
                if(rb && rb>lb){
                    /* Extract array name and index */
                    char aname[128]; int al=(int)(lb-arg);
                    if(al>=127) al=127;
                    memcpy(aname,arg,al); aname[al]=0;
                    char aidx[256]; int il=(int)(rb-lb-1);
                    if(il>=255) il=255;
                    memcpy(aidx,lb+1,il); aidx[il]=0;
                    const char *cn=safe_cname(aname);
                    add_var(aname,V_ARRAY);
                    /* Check if index is numeric or variable */
                    int is_num=1;
                    for(const char *q=aidx;*q;q++){ if(!isdigit((unsigned char)*q)){ is_num=0; break; } }
                    if(is_num && aidx[0]){
                        fprintf(out,"    { int __ui=atoi(\"%s\"); if(__ui>=0 && __ui<256) { __arr_%s[__ui]=NULL; } }\n",aidx,cn);
                    } else if(aidx[0]=='$'){
                        /* Variable index — use runtime evaluation */
                        char *e=translate_expr(aidx);
                        VarKind vk=get_var_kind(aidx+1);
                        if(vk==V_STR)
                            fprintf(out,"    { int __ui=atoi(%s); if(__ui>=0 && __ui<256) { __arr_%s[__ui]=NULL; } }\n",e,cn);
                        else
                            fprintf(out,"    { int __ui=%s; if(__ui>=0 && __ui<256) { __arr_%s[__ui]=NULL; } }\n",e,cn);
                        free(e);
                    } else {
                        /* String key — use assoc set with empty (assoc arrays don't support unset easily) */
                        fprintf(out,"    __sh_assoc_set(__arr_%s,\"%s\",\"\");\n",cn,aidx);
                    }
                    /* Re-pack array to remove NULL holes for ${#arr[@]}.
                     * Must iterate all 256 slots, not stop at first NULL,
                     * because unset creates holes in the middle. */
                    fprintf(out,"    { int __pi=0,__pj=0; for(;__pi<256;__pi++){ if(__arr_%s[__pi]){ if(__pi!=__pj) __arr_%s[__pj]=__arr_%s[__pi]; __pj++; } } __arr_%s[__pj]=NULL; }\n",cn,cn,cn,cn);
                }
            } else {
    ...
    if(!strcmp(cmd,"set")){
        ...
        for(int i=1;i<ac;i++){
            if(!strcmp(argv[i],"-e")) fprintf(out,"    __sh_set_e=1;\n");
                ...
                /* Update __sh_pos_args so $@/$* reflect the new values */
                for(int j=1;j<=n && j<=9;j++)
                    fprintf(out,"    __sh_pos_args[%d]=__sh_arg%d;\n",j-1,j);
                fprintf(out,"    __sh_args=__sh_pos_args;\n");
                break;
            }
        }
        return;
    ...
    if(!strcmp(cmd,"read")){
        ...
        for(int i=1;i<ac;i++){
            if(argv[i][0]=='-' && argv[i][1] && argv[i][1]!='-'){
                /* single or combined short flags: -r, -p PROMPT, -s, -a,
                 * and combinations like -rp / -pr / -rs. */
                int prompt_seen=0;
                for(const char *fc=argv[i]+1;*fc;fc++){
                    if(*fc=='r') continue;            /* raw: fgets is already raw */
                    else if(*fc=='a') read_array=1;
                    else if(*fc=='s') silent=1;
                    else if(*fc=='p') prompt_seen=1;
                    /* -t/-n/-d/-u/-i unsupported yet: treated as no-arg flags */
                }
                if(prompt_seen && !prompt && i+1<ac){ prompt=argv[i+1]; i++; }
                continue;
            }
        ...
        if(read_array){
            add_var(vars[0],V_ARRAY);
            fprintf(out,"    { char __rb[65536]; if(fgets(__rb,sizeof(__rb),stdin)){ int __rl=(int)strlen(__rb); if(__rl>0&&__rb[__rl-1]=='\\n')__rb[--__rl]=0;\n");
            fprintf(out,"    int __ai=0; const char *__ifs=getenv(\"IFS\"); if(!__ifs)__ifs=\" \t\";\n");
            fprintf(out,"    char *__sp=__rb; char *__tok=strtok(__sp,__ifs);\n");
            fprintf(out,"    while(__tok&&__ai<255){ __arr_%s[__ai++]=strdup(__tok); __tok=strtok(NULL,__ifs); } __arr_%s[__ai]=NULL;\n",safe_cname(vars[0]),safe_cname(vars[0]));
            fprintf(out,"    } }\n");
            return;
        }
        /* -p PROMPT: bash semantics — prompt goes to stderr, displayed
         * only when stdin is a terminal. Prompt word is expanded via
         * emit_word (supports $var interpolation in the prompt). */
        if(prompt){
            char *pw=emit_word(out,prompt);
            fprintf(out,"    if(isatty(0)){ fputs(%s,stderr); fflush(stderr); }\n",pw);
            free(pw);
        }
        /* -s silent: disable echo while reading (termios), then restore
         * and print the newline bash emits after silent reads. */
        if(silent){
            fprintf(out,"    { int __tt=isatty(0); struct termios __ot;\n");
            fprintf(out,"    if(__tt){ struct termios __t; if(tcgetattr(0,&__t)==0){ __ot=__t; __t.c_lflag&=~(ECHO); tcsetattr(0,TCSANOW,&__t); } else __tt=0; }\n");
        }
        fprintf(out,"    { char __rb[65536]; if(fgets(__rb,sizeof(__rb),stdin)){ int __rl=(int)strlen(__rb); if(__rl>0&&__rb[__rl-1]=='\\n')__rb[--__rl]=0;");
        if(nv==1){
        ...
        fprintf(out,"    } }\n");
        if(silent){
            fprintf(out,"    if(__tt){ tcsetattr(0,TCSANOW,&__ot); fputs(\"\\n\",stderr); }\n    }\n");
        }
        return;
    }
    if(!strcmp(cmd,"exit")){
        if(ac>1){
            /* Check if argument is a pure number (possibly quoted) */
            const char *arg=argv[1];
            /* Strip surrounding quotes if present */
            char stripped[256];
            if(arg[0]=='"' && arg[strlen(arg)-1]=='"'){
                size_t l=strlen(arg)-2;
                if(l<sizeof(stripped)){ memcpy(stripped,arg+1,l); stripped[l]=0; arg=stripped; }
            }
            int is_num = (arg[0] != 0);
            for(const char *p=arg;*p;p++){
                if(!isdigit((unsigned char)*p) && *p!='-'){ is_num=0; break; }
            }
            if(is_num){
                /* exit N — direct integer */
                fprintf(out,"    exit(%s);\n",arg);
            } else {
                /* exit $var or exit "$(cmd)" — use atoi */
                char *w=emit_word(out,argv[1]);
    ...
    /* ---- fallback: user-defined function call OR system command ---- */
    {
        if(is_user_func(cmd)){
            ...
            /* Check for bash -c / sh -c pattern: need to quote the script argument */
            int is_bash_c = (ac>=3 && !strcmp(argv[0],"bash") && !strcmp(argv[1],"-c"));
            int is_sh_c   = (ac>=3 && !strcmp(argv[0],"sh") && !strcmp(argv[1],"-c"));
            fprintf(out,"    { char __cmd%d[8192]; int __cl=0;\n",id);
            for(int i=0;i<ac;i++){
                if(i>0){ fprintf(out,"    __cmd%d[__cl++]=' ';\n",id); }
                /* For bash -c / sh -c, the script argument (i>=2) needs quoting */
                if((is_bash_c||is_sh_c) && i>=2){
                    /* Quote the argument to preserve it as a single shell string */
                    char *w=emit_word(out,argv[i]);
                    fprintf(out,"    __cmd%d[__cl++]='\\'';\n",id);
                    fprintf(out,"    __cl+=snprintf(__cmd%d+__cl,sizeof(__cmd%d)-__cl,\"%%s\",%s);\n",id,id,w);
                    fprintf(out,"    __cmd%d[__cl++]='\\'';\n",id);
                    free(w);
                } else {
...
void emit_node(FILE *out, Node *n){
    if(!n) return;
    /* Track line number for error reporting (e.g. division by zero) */
    if(n->lineno > 0) fprintf(out, "    __sh_lineno=%d;\n", n->lineno);
    switch(n->type){
    ...
    case NODE_ASSIGN:{
        ...
        if(n->lineno==-1 && n->rhs && n->rhs[0]=='('){
            ...
            while(*p && *p!=')'){
                ...
                if(*p=='"'){ p++; while(*p&&*p!='"'&&ei<(int)sizeof(elem)-1){
                    if(*p=='\\' && *(p+1)){
                        char next=*(p+1);
                        if(next=='$'||next=='"'||next=='\\'||next=='`'||next=='\n') p++; /* skip backslash */
                    }
                ...
                elem[ei]=0;
                { char *esc=c_escape_literal(elem);
                fprintf(out,"    __arr_%s[__al++]=\"%s\";\n",safe_cname(n->lhs),esc);
                free(esc); }
            }
        ...
        if(n->lineno < -1000){
            /* command not found error: x-=3 → "script: line N: x-=3: command not found" */
            int orig_lineno = -n->lineno - 1000;
            fprintf(out,"    fprintf(stderr,\"%%s: line %%d: %%s: command not found\\n\",__sh_script_name[0]?__sh_script_name:\"shell2c\",%d,\"%s\");\n",orig_lineno,n->rhs?n->rhs:"");
            fprintf(out,"    __exit_status=127;\n");
            break;
        }
        if(n->lineno==-2){
            /* string append: var+=value — bash does string concat for str vars, arith add for int vars */
            VarKind vk = get_var_kind(n->lhs);
            const char *cn=safe_cname(n->lhs);
            if(vk==V_INT){
                /* declare -i var: n+=5 → n = n + 5 (arithmetic) */
                char *e=translate_num_operand(n->rhs);
                fprintf(out,"    %s+=%s;\n",cn,e);
                free(e);
            } else if(strchr(n->rhs,'$')){
        ...
        if(n->rhs && n->rhs[0]=='('){
            ...
            while(*p && *p!=')'){
                ...
                int saw_quotes=0; /* track quoted elements (including empty "") */
                while(*p && *p!=' ' && *p!='\t' && *p!=')'){
                    if(*p=='"'){ p++; saw_quotes=1; while(*p&&*p!='"'){
                        /* In bash double quotes, \ is only special before $ ` " \ and newline.
                         * Before other chars, \ is literal. */
                        if(*p=='\\' && *(p+1)){
                            char next=*(p+1);
                            if(next=='$'||next=='"'||next=='\\'||next=='`'||next=='\n'){
                                p++; /* skip backslash, keep next char */
                            }
                            /* else: keep backslash literally */
                        }
                        q[k++]=*p++;
                    } if(*p)p++; }
                    else if(*p=='\''){ p++; saw_quotes=1; while(*p&&*p!='\'') q[k++]=*p++; if(*p)p++; }
                    else if(*p=='$' && (*(p+1)=='{' || *(p+1)=='(')){
                ...
                q[k]=0;
                /* Always add element if it was quoted (even if empty) or non-empty */
                if(k>0 || saw_quotes){ ni++; }
            }
            fprintf(out,"    /* array %s */\n",safe_cname(n->lhs));
            fprintf(out,"    {\n");
            ...
            for(int i=0;i<ni;i++){
                ...
                if(items[i][0]=='$' && items[i][0]!='"' && items[i][0]!='\''){
                    ...
                    int id=tmp_id++;
                    fprintf(out,"    { const char *__tw_%d=__sh_fmt_dyn(\"%s\"",id,er.fmt);
                    for(int ai=0;ai<er.nargs;ai++) fprintf(out,",%s",er.args[ai]);
                    fprintf(out,");\n");
                    expand_free(&er);
                    fprintf(out,"    char *__sp=(char*)__tw_%d;\n",id);
            ...
            } else if(dollar_outside_squotes(rhs)){
                ExpandResult er; expand_string(rhs,&er);
                add_var(n->lhs,V_STR);
                /* Route through dynamic pool temp: snprintf(dest,...,dest,...)
                 * with overlapping src/dest is UB (self-append BIG="$BIG x"). */
                fprintf(out,"    { const char *__t=__sh_fmt_dyn(\"%s\"",
                        er.fmt);
                for(int i=0;i<er.nargs;i++) fprintf(out,",%s",er.args[i]);
                fprintf(out,"); strncpy(%s,__t,sizeof(%s)-1); %s[sizeof(%s)-1]=0; }\n",
                        safe_cname(n->lhs),safe_cname(n->lhs),safe_cname(n->lhs),safe_cname(n->lhs));
                expand_free(&er);
            } else {
                /* Literal string (no variable expansion needed).
                 * Use full mixed-quote unescape to handle cases like
                 * 'It'\''s' (→ It's) and 'literal $var' (→ literal $var) */
                add_var(n->lhs,V_STR);
                int hd = 0;
                char *unescaped = unescape_mixed_quotes(rhs, &hd);
                const char *val = unescaped;
                /* Scatter-gather protection if obfuscate mode */
                extern int g_obfuscate;
                if(g_obfuscate && strlen(val) >= 6){
                    emit_scatter_string(out, safe_cname(n->lhs), val, 65536);
                } else {
                    char *esc=c_escape_literal(val);
                    fprintf(out,"    strncpy(%s,\"%s\",sizeof(%s)-1);\n",
        ...
        for(int i=0;i<n->argc;i++){
            ...
            if(eq){ *eq=0;
                ...
                if(vk==V_INT){
                    ...
                    fprintf(out,"    %s=%s;\n",cn,e);
                    free(e);
                } else if(vk==V_ARRAY || vk==V_ASSOC || eq[1]=='('){
                    /* array assignment */
                    ...
                    if(val[0]=='"' || val[0]=='\''){
                        ...
                        char inner[1024]; int il=0;
                        const char *vp=val+1;
                        while(*vp && *vp!=val[0] && il<(int)sizeof(inner)-1){
                            if(val[0]=='"' && *vp=='\\' && *(vp+1)){
                                char next=*(vp+1);
                                if(next=='$'||next=='"'||next=='\\'||next=='`'||next=='\n') vp++;
                            }
                        ...
                        inner[il]=0;
                        if(strchr(inner,'$')){
                            /* expand variables (pool temp avoids overlap UB) */
                            ExpandResult er; expand_string(inner,&er);
                            fprintf(out,"    { const char *__t=__sh_fmt_dyn(\"%s\"",er.fmt);
                            for(int ai=0;ai<er.nargs;ai++) fprintf(out,",%s",er.args[ai]);
                            fprintf(out,"); strncpy(%s,__t,sizeof(%s)-1); %s[sizeof(%s)-1]=0; }\n",cn,cn,cn,cn);
                            expand_free(&er);
                        } else {
                            char *esc=c_escape_literal(inner);
                            fprintf(out,"    strncpy(%s,\"%s\",sizeof(%s)-1);\n",cn,esc,cn);
                            free(esc);
                        }
                    } else if(strchr(val,'$')){
    ...
    case NODE_FOR:{
        if(n->for_c_style){
            ...
            if(do_array_iter){
                ...
                fprintf(out,"    {\n");
                fprintf(out,"    char __sbuf[65536]; strncpy(__sbuf,%s,sizeof(__sbuf)-1); __sbuf[sizeof(__sbuf)-1]=0;\n",vname);
            ...
            for(int i=0;i<n->for_len;i++){
                /* Check for glob chars, but skip ${...} expressions and double-quoted strings */
                const char *g=n->for_list[i];
                int in_dollar_brace=0;
                int in_dquote=0;
                for(;*g;g++){
                    if(*g=='"'){ in_dquote=!in_dquote; continue; }
                    if(*g=='$'&&g[1]=='{'){ in_dollar_brace=1; g++; continue; }
                    if(in_dollar_brace&&*g=='}'){ in_dollar_brace=0; continue; }
                    if(in_dollar_brace||in_dquote) continue;
                    if(*g=='*'||*g=='?'||(*g=='['&&g[1]!='A'&&g[1]!='a')){ has_glob=1; break; }
                }
                if(has_glob) break;
            }
            if(has_glob && n->for_len<=16){
                ...
                /* ${...}, $@, $*, "$@", "$*" — expand to string then split */
                char *w=emit_word(out,n->for_list[0]);
                fprintf(out,"    char __fl%d_buf[8192]; snprintf(__fl%d_buf,sizeof(__fl%d_buf),\"%%s\",%s);\n",
                    n->lineno,n->lineno,n->lineno,w);
                free(w);
                fprintf(out,"    const char *__fl%d[256]; int __nfl%d=0;\n",n->lineno,n->lineno);
                fprintf(out,"    { char *__s=__fl%d_buf; char *__tk=strtok(__s,\" \");\n",n->lineno);
                fprintf(out,"      while(__tk&&__nfl%d<255){__fl%d[__nfl%d++]=__tk;__tk=strtok(NULL,\" \");} }\n",
                    n->lineno,n->lineno,n->lineno);
                fprintf(out,"    __fl%d[__nfl%d]=NULL;\n",n->lineno,n->lineno);
            } else {
    ...
    case NODE_WHILE:{
        ...
        if(pending_pipe_cmd){
            ...
            free_node(pending_pipe_cmd); /* detached node: freed at emission */
        ...
        if(read_start){
            ...
            while(*p && nv<8){
                ...
                if(*p=='-'){
                    int pflag=0;
                    p++;
                    while(*p&&*p!=' '){ if(*p=='p')pflag=1; p++; }
                    /* -p consumes the next word (the prompt) */
                    if(pflag){ while(*p==' ')p++; while(*p&&*p!=' ')p++; }
                    continue;
                }
                int vl=0;
    ...
    case NODE_SELECT:{
        ...
        fprintf(out,"    char __sl[65536]; if(!fgets(__sl,sizeof(__sl),stdin)) break;\n");
    ...
    case NODE_EXIT:
        if(n->exit_str){
            /* Check if exit_str is a pure number */
            const char *es=n->exit_str;
            char estripped[256];
            if(es[0]=='"' && es[strlen(es)-1]=='"'){
                size_t l=strlen(es)-2;
                if(l<sizeof(estripped)){ memcpy(estripped,es+1,l); estripped[l]=0; es=estripped; }
            }
            int es_num = (es[0]!=0);
            for(const char *p=es;*p;p++){
                if(!isdigit((unsigned char)*p) && *p!='-'){ es_num=0; break; }
            }
            if(es_num){
                fprintf(out,"    exit(%s);\n",es);
            } else {
                char *w=emit_word(out,n->exit_str);
                fprintf(out,"    exit(atoi(%s));\n",w); free(w);
            }
        } else {
...
void emit_functions(FILE *out, Node *n){
    ...
    if(n->type==NODE_FUNC){
        ...
        for(int i=1;i<=max_arg;i++)
            fprintf(out,"    char __sh_arg%d[65536]=\"\"; if(__sh_argc>=%d)strncpy(__sh_arg%d,__sh_args[%d-1],65535);\n",
                    i,i,i,i);
