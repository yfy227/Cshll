...
转译器本身是一个约 5,200 行的单文件 C 程序（`shell2c.c`），将 shell 脚本解析为 AST，并生成带有丰富嵌入式运行时库的 C 代码。

### 核心特性

- **10 层架构**：从分词器到运行时库，每层职责清晰
- **117 个原生内置命令**：直接生成 C 代码，零进程开销
- **全系统命令支持**：未识别命令自动通过 `system()` 透传
- **零编译器警告**：`-Wall -Wextra` 全部通过
- **13/13 测试通过**：与 bash 输出精确匹配
- **性能优化**：for 循环零堆分配，函数调用栈分配，临时缓冲区缩减 75%
...
深度优化：多层架构、117+ 内置命令、fd 保存/恢复纪律、用户函数命令替换、性能优化（零堆分配 for 循环、栈分配函数调用）、全系统命令透传、优雅 C 代码生成。
