...
> **定位说明**：VM 模式的目标是**脚本保护/防逆向**（字节码加密 + 控制流平坦化 + 反分析设计），不是性能优化。它适合需要隐藏脚本逻辑的场景；普通转译（默认模式）生成可读性更好的 C 代码。VM 路径与转译路径是两套执行模型，行为一致性**尚未收敛**——2026-08-22 实测抽样（6 个代表性 e2e 脚本 vs bash）：**2/6 一致**，主要缺口在文件重定向语义（VM 路径使用合成文件名如 `/tmp/s2c_redir_00.txt`）。生产使用请以默认转译路径为准。
